#!/usr/bin/env python3
"""Status line: the always-on context gauge, and the sensor record other tools read.
...
The gauge colours tokens left green, yellow or red by remaining-token
thresholds: built-in defaults, or - when a gauge publisher is active for this
session (sensor.publisher()) - its published thresholds, and then also its
epoch number and its words for the two bands. Standalone, the line shows no
epoch and no band words.
...
def gauge_text(pct, size, tok, pub):
    """The context gauge: bar, percent, tokens left, and - only when a gauge
    publisher is active (`pub`, from sensor.publisher()) - its epoch and its
    word for the band. Colour: green while left > due, yellow while left >
    hard, else red, with due/hard interpolated for this window over the
    publisher's anchors or DEFAULT_ANCHORS."""
    p = int(pct)
    left = max(size - tok, 0)
    th = S.thresholds(size, pub["anchors"] if pub else S.DEFAULT_ANCHORS)
    ...
    tail = ""
    if pub:
        if pub["epoch"] is not None:
            tail += f"  e{pub['epoch']}"
        word = ("" if left > th["due"] else
                pub["labels"]["due"] if left > th["hard"] else pub["labels"]["hard"])
        if word:
            tail += f"  ·  {word}"
    return f"{color}{bar}\033[0m {p}%  {left // 1000}k left{tail}"
...
def main():
    ...
    try:
        ...
        print(""); return
    now = time.time()  # when the payload was read: the records' `at`
    ...
    # record that would tell a reader there is nothing left - a fractional
    # size would otherwise truncate to 0. The used percentage is clamped to
    # 0-100.
    ...
    if pct is not None:
        try:
            gauge = gauge_text(pct, size, tok, S.publisher(sid) if sid else None)
        except Exception:
            gauge = "ctx --"
