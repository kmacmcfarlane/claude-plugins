...
manifest header and its Holds lines, so the opener is what tells the next session to read
the whole file); and the one or two facts that changed since the manifest was written —
pull these from the Holds lines first, then the drift note or the `Aware of` lines you just
wrote (the lean path has no drift note; use Holds and `Aware of`), never restate the whole
manifest.
