...
4. **Decide on settings:**
   ...
   - Optional fields: `model`, `license`, `compatibility`, `metadata` (author, version, mcp-server, category, tags). House rule: every skill declares the five keys in the structure below; only `context` and these optional fields may be added (see `references/frontmatter-reference.md`).
