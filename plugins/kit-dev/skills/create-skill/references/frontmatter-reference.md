...
- **Required**: every skill declares these two keys: `name`, `description`.
- **Allowed**: any other field Claude Code's skills docs (code.claude.com/docs/en/skills)
  define, each optional: `disable-model-invocation`, `allowed-tools`, `argument-hint`,
  `when_to_use`, `arguments`, `user-invocable`, `disallowed-tools`, `model`,
...
(`name`, `description`, `license`, `compatibility`, `metadata`, `allowed-tools`). A skill
that declares `disable-model-invocation` or `argument-hint` (most here do) is outside that
set, so it is a Claude Code skill only.

Leaving out `disable-model-invocation`, `allowed-tools` and `argument-hint` is the desired
default, not a gap: the skill is model-invocable, takes free-form arguments and
pre-approves no tools. Declare one only when the skill needs a non-default value.
