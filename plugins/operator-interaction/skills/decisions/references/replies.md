...
3. **Echo** every reply that is not an exact `N: letter`, in one italic line before you act:
   *Read as: 74 → dig into (who imports the package).* The echo is how a misreading is caught
   in one turn instead of after the damage.
4. Act — except on a ⚠ one-way choice (the read-back below).

When a reply mixes a letter and words (`74: b, keep and alias`), the letter decides; the
words are checked against the option and a mismatch is asked about, not guessed.
...
*Read as: 74 → (b) keep the name and add an alias. This one is one-way — confirm and I'll go
ahead, or change it.*

A confirmation in any words counts ("yes", "go", "confirmed"). The read-back applies only to
replies that choose an option; `later`, `tell me`, `expand`, `dig into` and `drop` on a ⚠
decision are echoed normally.

**`later [when]` — decide later.** Nothing happens except that the decision waits. Set its
wake:

- a time — "tomorrow morning", "after 14:00";
- an event — "when the CI fix lands", "after the release";
...
*Read as: 1 → later (when the CI fix lands). Note: the lock expires at 15:30; if the step is
still paused then, it fails and needs a full two-hour re-run. Keep that, or pick a time before
15:30?*
...
**`dig into [what]` — investigate first.** A bounded investigation: state its cost up front
...
your next update: *Read as: 6 → you decide. I'll take (a), because it matches the existing
entries; noted in my next update.* **Refused on ⚠ one-way:** *74 is one-way — it needs your
own choice. I recommend (b), because …. Which do you want?*

**`drop` — retire it.** *Read as: 9 → drop. Retired; it won't come back.* If something still
...
decision and raise the new one under a new number that points back: *Read as: a reframe of 3
— new decision 15 replaces it.* Then show 15 at its level.
...
*Accepted 5, 6, 8. Skipped: 7 (⚠ one-way, needs your own answer), 9 (your preference, no
recommendation).*
...
- **A status-quo default** may be stated on the card, because it changes nothing: *If
  unanswered: I leave the branch as it is and carry on with other work.*
