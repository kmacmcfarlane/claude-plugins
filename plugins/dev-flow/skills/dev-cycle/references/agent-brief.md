...
- `argument-hint`, when present, is always a double-quoted string, since unquoted a
  value starting with `[` is a YAML flow sequence: a list, or with a second `[...]` group
  a parse error that drops the skill in strict loaders.
...
`verdict:` line (`record-lines.md`). Findings are the reviewer's report,
