...
**Enforcement:** claude-kit ships a PreToolUse hook (`hooks/checkout_guard.py`) that blocks `Edit`/`Write`/`NotebookEdit` on git-tracked files when the session's cwd is a main checkout — the direction the harness's own worktree guard does not cover. Untracked files, paths under `.claude-sandbox/` or `.claude/`, worktree cwds, and non-git cwds pass untouched, and any git error fails open (ralph runs in a worktree, so it is exempt by construction). Escape hatches: set `CLAUDE_KIT_ALLOW_CHECKOUT_EDITS=1` in the environment, or create a `.claude/allow-checkout-edits` marker file in the repo root for a standing per-repo opt-out. Bash writes (sed, heredocs) are not guarded.
