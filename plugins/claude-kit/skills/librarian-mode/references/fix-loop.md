...
- `NEEDS_CHANGES`: hand the findings, verbatim, to the **implementer** — resume the same
  ...
  agent keeps its model); on a tier bump, or if gone, re-dispatch with the full brief,
  the findings and the fix-round clause from `references/agent-brief.md`. Tell
  ...
  **reviewer** — re-dispatched fresh on a tier bump of either role, or if gone — with
  the re-review variant in `references/review-brief.md`, pasting the
