...
## Review round 1 — CLEAR (opus) at 13c5348
- nothing on the block path reads epoch_end_tokens; upgrade scenario in a temp config dir: no lost marker, no double settings write (and --remove now marks the dir the hook reads — a fix).
- lows (landed as CLEAR): owner.py cache-path name skips CC's id rule (marketplace names with "."); hatch path quoting vs "!" in interactive bash (shlex.quote); playbook snippet takes entry [0] regardless of scope.
## Landed
- beb5a81.
