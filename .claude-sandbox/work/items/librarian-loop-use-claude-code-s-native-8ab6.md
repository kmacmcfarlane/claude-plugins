...
answer 63: (a) adopt dynamic /loop as the fallback heartbeat, gated by an in-flight check (operator 2026-09-22)
operator requirements with the answer: (1) automatic start is preferred — the librarian should arm the loop itself if it can, not hand the operator a command to run; establish whether an agent can start /loop (the Skill tool lists it; ScheduleWakeup's contract says "the user invoked /loop") and record the answer. (2) The loop must BACK OFF when there is no work: SendMessage from other agents and the operator's own turns are the async wake path, and work almost never arrives from anywhere else, so an empty queue means long delays (escalating back-off toward the 3600 s cap), not frequent empty ticks. (operator 2026-09-22)
