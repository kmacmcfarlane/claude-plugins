...
- **Hook location**: `plugins/claude-kit/hooks/<name>.py` — registered in `plugins/claude-kit/hooks/hooks.json`, which lists each hook under its event (`PreToolUse`, `UserPromptSubmit`, `SessionStart`, `Stop`, …) with a `matcher` and a `command` that names the script through the plugin root, `python3 "${CLAUDE_PLUGIN_ROOT}/hooks/<name>.py"` — never a relative path, since the command runs with no guaranteed working directory. Tests live in `plugins/claude-kit/hooks/tests/` and run with `python3 -m unittest discover -s tests -q` from the hooks dir. Hooks, status lines and `settings.json` writes belong only in the plugin whose stated aim is that behavior — claude-kit today.
