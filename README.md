...
now live state; `dev-flow` — shipped at Phase 3, its librarian store at `${CLAUDE_CONFIG_DIR}/claude-kit/librarian/` live state, not keyed by the plugin name; `work-items` — shipped
...
put their decision-channel decisions to the operator per that skill (its content floor,
levels, order, echo and read-back). Without it, the librarian keeps its numbered
`decisions needed:` list, and a standalone `dev-cycle` keeps its own channel
(AskUserQuestion for one decision, a numbered list for several).
...
| Skill | Description |
|---|---|
| `work-items` | `wi` — ready-ranked queue, atomic claims, handoff blocks, TODO.md importer, backlog-yaml bridge, and `wi estate`, the read-only sweep of every repo's store beside this one |
| `work-review` | On-demand, read-only review of the work items across your repos: security items first, each repo's state in plain words, the decisions waiting on you, and what to do next with the reason — never scheduled, and it never answers another repo's decisions |
